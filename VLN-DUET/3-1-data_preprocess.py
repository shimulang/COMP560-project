...
anno_files = ["datasets/R2R/annotations/pretrain/train_add_heading_caption_add_elevation_openai_generate_instructions_v6.jsonl"]
...
for anno_file in anno_files:
    ...
    with jsonlines.open(anno_file, 'r') as f:
        for index, item in enumerate(f):
            ...
            # # 使用 ast.literal_eval 将字符串转化为 list
            # cleaned_instructions = ast.literal_eval(cleaned_sentences)
            # if isinstance(generate_instructions, str):
            #
            #     # 提取出 [] 中的内容
            #     cleaned_sentences = generate_instructions.strip().strip('"')
            #     cleaned_instructions = [generate_instructions]
            pattern = r'"([^"]*)"'
        ...
        # 保存到新的jsonl文件中
    with jsonlines.open("datasets/R2R/annotations/pretrain/train_openai_chatgpt4_generate_instructionsv6.jsonl", 'w') as f:
