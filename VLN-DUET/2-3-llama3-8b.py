...
# ？
# url = "http://10.20.0.135:8000/v1/chat/completions"

client = OpenAI(
    base_url="http://10.20.0.135:8000/v1",
    api_key="Langji@123",
)
...
for anno_file in anno_files:
    ...
    indices = set()
    # 打开新的jsonl文件，准备读取数据
    with jsonlines.open(anno_file.replace('.jsonl', '_llama3_70B_generate_instructions_v7.jsonl'), 'r') as infile:
        # 遍历新的jsonl文件中的每一行
        for item in infile:
            # 将这个索引添加到集合中
            indices.add(item['path_id'])

    with jsonlines.open(anno_file.replace('.jsonl', '_llama3_70B_generate_instructions_v7.jsonl'), 'a') as outfile:
        # 保存每个item到新的jsonl文件中
        update_item = []
